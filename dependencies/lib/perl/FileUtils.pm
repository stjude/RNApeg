...
sub log_stdout_stderr {
  my %options = @_;
  my $bn = $options{"-file"} || "log";

  foreach my $ext (qw(out err)) {
    my $fn = join ".", $bn, $$, $ext;
    if ($ext eq "out") {
      open(STDOUT, ">" . $fn) || die "can't write to $fn: $!";
    } elsif ($ext eq "err") {
      open(STDERR, ">" . $fn) || die "can't write to $fn: $!";
    } else {
      die;
    }
  }
...
sub sort_delimited_file {
  my (%options) = @_;
  my @infiles;
  if (my $fn = $options{"-file"}) {
    push @infiles, $fn;
  } elsif (my $set = $options{"-files"}) {
    push @infiles, @{$set};
  } else {
    die "-file or -files";
  }
  ...
  my $numeric = $options{"-numeric"};

  my $fh = universal_open($infiles[0]) || die;
  my $header_line = <$fh>;
  chomp $header_line;
  my @h = split $delimiter, $header_line;
  my $i = 0;
  my %h2i = map {$_, ++$i} @h;
  my $field_number = $h2i{$field} || die "can't find header entry for $field";
  my $outfile_tmp = $outfile . ".tmp";
  open(SDFTMP_OUT, ">" . $outfile_tmp) || die;
  printf SDFTMP_OUT "%s\n", $header_line;
  close SDFTMP_OUT;

  my $sort_cmd = sprintf '|sort -t "%s" -k%d,%d', $delimiter, $field_number, $field_number;
  $sort_cmd .= " -n" if $numeric;
  $sort_cmd .= ' | uniq' if $uniq;
  $sort_cmd .= sprintf ' >> %s', $outfile_tmp;
  printf STDERR "sort cmd: %s\n", $sort_cmd;

  open(SDFTMP_OUT, $sort_cmd) || die;

  my %hl;
  foreach my $infile (@infiles) {
    my $fh = universal_open($infile) || die;
    my $h = <$fh>;
    chomp $h;
    $hl{$h} = 1;
    die "header lines disagree" if scalar keys %hl > 1;
    # skip header line
  ...
  close SDFTMP_OUT;
  die if $?;
  rename($outfile_tmp, $outfile) || die "can't rename $outfile_tmp to $outfile: $!";
}
